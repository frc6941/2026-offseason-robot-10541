...
public class SwerveCommands {
    ...
    private static final Function<Double, Double> kJoystickCurveQuadratic =
            (x) -> x * x * Math.signum(x);
    ...
    public static Command driveToPose(
        ...
        return new SwerveDriveToPose(
                ...
                poseWorldTargetSupplier,
                velocityWorldRobotSupplier,
    ...
    public static Command resetAngle(Swerve swerve, Rotation2d rotation) {
        return Commands.runOnce(
                        () -> {
                            Pose3d poseWorldRobotCurr = swerve.getEstimatedPose();
                            Pose3d newPoseWorldRobotCurr =
                                    new Pose3d(
                                            poseWorldRobotCurr.getTranslation(),
                                            new Rotation3d(rotation));
        ...
        return Commands.runOnce(
                        () -> {
                            ...
                            Pose3d newPoseWorldRobotCurr =
                                    new Pose3d(
                                            new Pose2d(
                                                    poseWorldRobotCurr.toPose2d().getTranslation(),
                                                    rotation.get()));
                            swerve.resetEstimatedPose(newPoseWorldRobotCurr);
                        })
                .ignoringDisable(true);
    ...
    public static SysIdRoutine sysid(
            Swerve swerve, Velocity<VoltageUnit> rampVelocity, Voltage stepVoltage, Time timeout) {
        return new SysIdRoutine(
                new SysIdRoutine.Config(
                        ...
                        state -> {
                            Logger.recordOutput(
                                    "Sysid/Swerve/Voltage", swerve.getPreviouslyAppliedVoltage());
                            Logger.recordOutput(
                            ...
                            Logger.recordOutput(
                                    ...
                                    Math.hypot(speed.vxMetersPerSecond, speed.vyMetersPerSecond)
                                            * Math.signum(
                                                    swerve.getPreviouslyAppliedVoltage()
                                                            .in(Volts)));
                            Logger.recordOutput("Sysid/Swerve/State", state.toString());
                        }),
                new SysIdRoutine.Mechanism(swerve::runVoltage, null, swerve));
    }
}
