...
public class SwerveModuleIOSim implements SwerveModuleIO {
    ...
    private void initializePlants() {
        driveMotorSim =
                new DCMotorSim(
                        ...
                        config.driveMotor,
                        config.driveStdDevPos,
                        config.driveStdDevVel);
        steerMotorSim =
                new DCMotorSim(
                        LinearSystemId.createDCMotorSystem(
                                config.steerMotor,
                        ...
                        config.steerMotor,
                        config.steerStdDevPos,
                        config.steerStdDevVel);
    }
    ...
    public void updateInputs(SwerveModuleIOInputs data) {
        ...
        data.driveMotorTorqueCurrentAmpere =
                driveMotorSim.getTorqueNewtonMeters() / config.driveMotor.KtNMPerAmp;
        ...
        data.steerMotorTorqueCurrentAmpere =
                steerMotorSim.getTorqueNewtonMeters() / config.steerMotor.KtNMPerAmp;
    ...
    public void setDriveVelocity(LinearVelocity linearVelocityDes) {
        ...
        AngularVelocity angularVelocityDes =
                RotationsPerSecond.of(
                        linearVelocityDes.in(MetersPerSecond) / distancePerRotation.in(Meter));

        double fb =
                driveFB.calculate(
                        driveMotorSim.getAngularVelocityRadPerSec() / (2 * Math.PI),
                        angularVelocityDes.in(RotationsPerSecond));
    ...
    public void setSteerAngleAbsolute(Angle des) {
        ...
        double fb =
                steerFB.calculate(
                        Radian.of(steerMotorSim.getAngularPositionRad()).in(Radian),
                        des.in(Radian));
